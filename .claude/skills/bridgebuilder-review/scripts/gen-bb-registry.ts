...
// cycle-124 FR-3 (SDD §2.1, Flatline SKP-003): when a yaml entry carries the
// v3 `effective_input_ceiling` (cheval's pre-flight gate — exit 7 above it),
// BB's maxInput is that ceiling minus this headroom so the truncation engine
// never prepares a payload cheval would refuse. Entries without a ceiling
// keep `context_window`.
...
function flattenModels(providers: Record<string, YamlProvider>): FlatModelEntry[] {
  ...
  for (const provider of providerNames) {
    ...
    for (const modelId of modelIds) {
      ...
      // cycle-124 FR-3: an entry that declares a ceiling must declare one BB can
      // subtract the headroom from; anything else is a catalog error, not a
      // silently-clamped budget.
      if (m.effective_input_ceiling !== undefined) {
        if (
          !Number.isInteger(m.effective_input_ceiling) ||
          m.effective_input_ceiling <= CEILING_HEADROOM_TOKENS ||
          m.effective_input_ceiling > m.context_window
        ) {
          throw new ConfigError(
            `provider ${provider}, model ${modelId}: invalid effective_input_ceiling ${String(
              m.effective_input_ceiling,
            )} (must be an integer in (${CEILING_HEADROOM_TOKENS}, context_window=${m.context_window}])`,
          );
        }
      }
      entries.push({
