...
export function hasApprovedReviewQuality(response) {
    if (!response || response.errorState != null)
        return false;
    ...
    while ((event = walker.next())) {
        ...
        if (node.type === "paragraph" || node.type === "heading") {
            if (entering) {
                ...
                if (!containsHTML && block !== undefined)
                    lines.push(block);
                block = undefined;
            }
        }
...
export function combineReviewVerdicts(decisions) {
