...
function parseReviewignoreFile(repoRoot: string): string[] {
  ...
  try {
    ...
    const enriched = new Error(
      `repoRoot does not exist or is unreadable: ${repoRoot} (code=${code ?? "unknown"})`,
  ...
  if (!rootStat.isDirectory()) {
    const enriched = new Error(
      `repoRoot is not a directory: ${repoRoot}`,
    ) as NodeJS.ErrnoException;
    enriched.code = "EBADREPOROOT";
    throw enriched;
  }
  ...
  try {
    ...
    if (code === "ELOOP") {
      ...
      const enriched = new Error(
        `.reviewignore is a symbolic link — rejected to prevent symlink-follow-on-read TOCTOU (BB801-REVIEW-002 + BB801-R8): ${reviewignorePath}`,
      ) as NodeJS.ErrnoException;
  ...
  try {
    ...
    if (!fdStat.isFile()) {
      const enriched = new Error(
        `.reviewignore is not a regular file (BB801-REVIEW-002 fail-closed): ${reviewignorePath}`,
      ) as NodeJS.ErrnoException;
...
export function loadReviewIgnore(repoRoot?: string): string[] {
  if (!_loadReviewIgnoreDeprecationWarned) {
    ...
    process.stderr.write(
      "[bridgebuilder] DEPRECATED: loadReviewIgnore(repoRoot) is deprecated — " +
  ...
  try {
    ...
    process.stderr.write(
      `[bridgebuilder] WARN: legacy loadReviewIgnore swallowed .reviewignore error — see deprecation. Detail: ${(err as Error).message}\n`,
    );
  }
  ...
  if (loaDetection.isLoa) {
    if (selfReviewActive) {
      ...
      try {
        ...
        const bytesSaved = files.reduce(
          (sum, f) => sum + (f.patch ? new TextEncoder().encode(f.patch).byteLength : 0),
          0,
        );
        process.stderr.write(
          `[bridgebuilder] ERROR: .reviewignore anomaly in default-mode path — fail-closed (allExcluded=true). Fix the file to restore review. Code: ${code}, Detail: ${(err as Error).message} (BB801-001 iter-3 HIGH_CONSENSUS)\n`,
        );
        return {
          included: [],
          excluded: files.map((f) => ({
          ...
          allExcluded: true,
          loaBanner:
            `[Loa-aware: default-mode fail-closed — .reviewignore anomaly (${code}); ` +
