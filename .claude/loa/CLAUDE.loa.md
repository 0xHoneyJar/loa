...
| ALWAYS gate SOUL.md path / log overrides behind the test-mode env-var gate | `LOA_SOUL_LOG`, `LOA_SOUL_TEST_PATH`, `LOA_SOUL_TEST_CONFIG` are honored ONLY under bats / `LOA_SOUL_TEST_MODE=1` with a bats-detected ancestor. Mirrors L4 cycle-099 #761 + L6 sprint 6 CYP-F1 patterns. Production paths emit a stderr `WARNING: env override ignored` and use defaults. |
