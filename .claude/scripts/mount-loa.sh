...
sync_zones() {
  ...
  log "Pulling CLAUDE.md (Claude Code instructions)..."
  git checkout "$LOA_REMOTE_NAME/$LOA_BRANCH" -- CLAUDE.md 2>/dev/null || {
    warn "No CLAUDE.md in upstream, skipping..."
  }
...
create_upgrade_commit() {
  ...
  git add .claude .loa-version.json CLAUDE.md 2>/dev/null || true
