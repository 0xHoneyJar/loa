...
aggregate_capabilities() {
    ...
    while [[ $i -lt $skill_count ]]; do
        ...
        # Get schema_version
        local sv
        sv=$(_strict_or_default "0" "$pack_slug/$skill_slug capabilities.schema_version" yq eval '.capabilities.schema_version // 0' "$tmp_fm")
        ...
        for field in read_files search_code write_files web_access user_interaction agent_spawn task_management; do
            val=$(_strict_or_default "false" "$pack_slug/$skill_slug capabilities.$field" yq eval ".capabilities.$field // false" "$tmp_fm")
            if [[ "$val" == "true" ]]; then
        ...
        local exec_type
        exec_type=$(_strict_or_default "!!null" "$pack_slug/$skill_slug capabilities.execute_commands type" yq eval '.capabilities.execute_commands | type' "$tmp_fm")

        if [[ "$exec_type" == "!!bool" || "$exec_type" == "boolean" ]]; then
            val=$(_strict_or_default "false" "$pack_slug/$skill_slug capabilities.execute_commands" yq eval '.capabilities.execute_commands' "$tmp_fm")
            if [[ "$val" == "true" ]]; then
            ...
            if [[ "$agg_execute_commands" != "true" ]]; then
                ...
                # Merge allowed lists
                local new_allowed
                new_allowed=$(_strict_or_default "[]" "$pack_slug/$skill_slug capabilities.execute_commands.allowed" yq eval -o=json '.capabilities.execute_commands.allowed // []' "$tmp_fm")
...
process_pack() {
    ...
    if [[ -f "$manifest" ]]; then
        ...
        manifest_json=$(yq eval -o=json '{
            ...
            warn "Could not read construct.yaml in '$pack_slug' — skipping"
            return 1
        }
    ...
    if [[ -f "$construct_yaml" ]] && command -v yq &>/dev/null; then
        ...
        cy_name=$(_strict_or_default "" "$pack_slug construct.yaml name" yq eval '.name // ""' "$construct_yaml")
        cy_version=$(_strict_or_default "" "$pack_slug construct.yaml version" yq eval '.version // ""' "$construct_yaml")
        cy_description=$(_strict_or_default "" "$pack_slug construct.yaml description" yq eval '.description // ""' "$construct_yaml")
