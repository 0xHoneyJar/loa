...
# from `.run/merged-model-aliases.sh`, generated by Sprint 2B's
# model-overlay-hook.py. Best-effort: when the overlay isn't available
# (file missing AND hook regen failed), the framework-only resolver
# above remains the resolution path. Callers should try
# loa_overlay_resolve_provider_id FIRST, then resolve_provider_id, then
# the local MODEL_TO_ALIAS map below.
# shellcheck source=lib/overlay-source-helper.sh
source "$SCRIPT_DIR/lib/overlay-source-helper.sh"
# Initialize the overlay; failures are non-fatal — adapter falls through
# to model-resolver.sh resolution chain.
