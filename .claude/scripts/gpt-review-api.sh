...
EOF
  ...
  if [[ -z "${OPENAI_API_KEY:-}" ]]; then
    ...
    if [[ -f ".env" ]]; then
      env_key=$(grep -E "^OPENAI_API_KEY=" .env 2>/dev/null | tail -1 | cut -d'=' -f2- | tr -d '"' | tr -d "'" || true)
    ...
    if [[ -f ".env.local" ]]; then
      local local_key
      local_key=$(grep -E "^OPENAI_API_KEY=" .env.local 2>/dev/null | tail -1 | cut -d'=' -f2- | tr -d '"' | tr -d "'" || true)
      if [[ -n "$local_key" ]]; then
        env_key="$local_key"
