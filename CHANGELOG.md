...
- **Pre-release candidates through the post-merge pipeline (bug 20260923-27d899, sprint-bug-240)**: `semver-bump.sh` honours the topmost untagged CHANGELOG heading for two transitions — an rc heading of the version the commits warrant enters the prerelease (`2.0.0` → `2.0.0-rc.1`, `prerelease_transition.kind: enter`), a bare heading on an rc tag promotes it (`kind: promote`); `rc.N → rc.N+1` is unchanged, any other heading is ignored (a mismatched rc heading warns). The candidate JSON carries `prerelease`, `publish_candidate` ties it to the tag, and `phase_release` creates the GitHub release with `prerelease: true` for a prerelease version and verifies the flag on read-back. `sync-readme-version.sh` accepts `X.Y.Z-PRE.N` and renders the badge with shields' `--` escape. Runbook: `grimoires/loa/runbooks/post-merge-candidates.md` §Pre-release candidates. Tests first: 6 semver, 3 publication, 3 README-sync cases.
- **Prompt byte budgets and the 49-unit prompt audit** (framework-review §9, PRD FR-8): `tools/check-prompt-budget.sh` (skills ≤ 16,384 B incl. unconditionally-read resources, `CLAUDE.loa.md` ≤ 10,240 B, protocols ≤ 200,000 B) with `tools/prompt-keeplist.txt`, the no-history-in-rule-text / generated-block / protocol-refs / constraint-temp / golden-scope bats and `check-prompt-budget.yml`; `CLAUDE.loa.md` becomes a 10,222-byte kernel (Karpathy principles' full text lives in `karpathy-principles.md`); 13 skills cut to the floor with conditional procedures behind guarded `resources/*.md` pointers; `risk-analysis.md`, `upgrade-process.md`, `sprint-completion.md` archived (the verdict-trailer rule moved to `feedback-loops.md`); protocols 267 KB → 199 KB; every unit report under `grimoires/loa/a2a/sprint-237/prompt-audit/`. New agents `prompt-auditor` / `prompt-auditor-io`.
- **Coverage-first review and audit** (PRD FR-9): reviewing-code, auditing-security and the Flatline reviewer/skeptic personas report every finding with `file:line`, a failure scenario, severity and confidence — no minimum-count ritual; `verdict-derive.sh` gains the additive `excluded` / `excluded_confirmed` trailer fields (a speculative low-confidence HIGH may sit under `## Observations`; a CRITICAL never), `golden-path.sh` surfaces the count; effort defaults come from SKILL.md frontmatter via `model-adapter.sh resolve_effort()` (`--effort` arg > frontmatter > none) and `flatline-orchestrator.sh` maps review/skeptic → `xhigh`, score → `medium`; the dead `effort:` config block is gone.
- **Eval A/B harness**: `evals/harness/execute-agent.sh` runs a headless `claude -p` agent against a materialized prompt tree (model, effort, tree sha and dirty flag recorded per trial); a 10-PR review corpus built from inverted `fix(...)` commits with hidden manifests (`evals/fixtures/review-prs/`), implement-discipline fixtures, deterministic graders (anchored recall / false positives, verdict consistency, test-first/surgical/zone/tests-pass), `compare.sh` freshness rule and `--ab` gates. Sprint 3 measured arm A (Sprint-2 prompts) vs arm B/B2 (audited prompts): audit recall and clean-PR false positives unchanged, review recall within tolerance after one iteration, one clean-PR HIGH remains, audit tokens per call −24 % (gate target was −50 %); results under `grimoires/loa/a2a/sprint-237/ab/`.
- **CI: the backup-sibling check ignores deletions** (`.github/workflows/no-backup-files.yml`, `--diff-filter=d`): deleting a stale `*.constraint-XXXXXX` twin is the remedy the check asks for, not a violation. The cycle's per-sprint a2a record (reports, feedback, dissent JSON, prompt-audit units, A/B arms) lives on the never-merged branch `record/cycle-124-a2a`; the template repository keeps `grimoires/loa/a2a/sprint-*` and `a2a/trajectory/` out of `main` by policy.
- **Aleph is opt-in (bug 20260922-13a3d1, sprint-bug-239)**: Aleph is a third-party, vendored component and no longer constrains Loa. One switch, `aleph.enabled` in `.loa.config.yaml` (default `false`; `LOA_ALEPH_ENABLED=1` for tests/CI), read by `.claude/scripts/lib/aleph-opt-in.sh`: `mount-submodule.sh`/`update-loa.sh` skip the Aleph refresh and verification, `check-loa.sh` skips `check_aleph_integrity`, the three `tests/unit/aleph-*.bats` suites skip themselves, and `aleph-bundle-integrity.yml` / `aleph-release-sync.yml` gain a `gate` job so their verify/prepare/propose jobs run only when enabled. Opted-in behaviour is unchanged. Motivation: PR #1266 went red on `Verify install` and on the required `Shell Tests` job because Loa's own capabilities lint added a frontmatter block to the Aleph-managed `loa-aleph/SKILL.md`; the block stays. Tests: `tests/unit/aleph-opt-in.bats` (AO-1..AO-7).
- **Bounded session memory (PRD FR-10, Sprint 4)**: `.claude/scripts/notes-guard.sh` — `check` (100 KiB warn / 200 KiB block as literals; `--delta` is direction-aware, so compaction never blocks), `read` (heading-based default: every `## Blockers`, the newest `## Session Continuity`, the 3 newest `## Decision Log` blocks by heading date, ≤ 69,632 B with a `read --full` footer, loud template-drift fallback, never empty; `--full` = the whole file), `rotate` (archive to `grimoires/loa/archive/notes/NOTES-<UTC>.md`, fsynced before the live rewrite, existing target refused with exit 4, tmp + mv, never stashes). Fences: `.claude/hooks/safety/notes-size-guard.sh` (PreToolUse Write/Edit/MultiEdit, realpath-matched against `$LOA_GRIMOIRE_DIR/NOTES.md`, denies only a write that would grow the file at/over 200 KiB; wired in `settings.json` and `hooks/settings.hooks.json` behind `hook-guard.sh`), `FR-NOTES` in `block-destructive-bash.sh` for `>> …grimoires/loa/NOTES.md` appends (blocked only while `check` exits 3; `rotate` is never intercepted), and a writer gate in `update-notes-learnings.sh` (exit 3, nothing written; the script now honours `LOA_GRIMOIRE_DIR`). Readers: the session-continuity Level 1/3 recipes, `protocols/ride-translation.md` and translating-for-executives go through `notes-guard.sh read`; `structured-memory.md`, `context-engineering.md` and `hooks-reference.md` document the thresholds and the accepted bypass classes; decision memo `grimoires/loa/reports/2026-09-17-notes-vs-memory-tool.md` (keep NOTES.md; do not adopt `memory_20250818`). Tests first: `tests/unit/notes-guard.bats` (12), `tests/unit/notes-size-guard.bats` (10), three `FR-NOTES` cases in `block-destructive-bash.bats`, three FR-10 cases in `notes-template.bats` (thresholds documented; unbounded-`cat` reader set empty); fixtures are generated at test time by `tests/fixtures/notes/make-large-notes.sh`, nothing large is committed.
- **Unit suite no longer writes to the production ledgers (Sprint 4 goal G-6)**: two suites still reached cheval without ledger isolation — `tests/unit/cheval-preflight-gate.bats` P17 unset the API keys and expected a failure, but on a host with a CLI hop the chain walked for real (one `codex-headless` dispatch of ~25k input tokens per `bats tests/unit/` run); it now runs through `--mock-fixture-dir` and, like `tests/unit/lib-curl-fallback-flatline-chat.bats` (mock round-trip through a shim), exports `LOA_MODELINV_LOG_PATH` / `LOA_COST_LEDGER_PATH` into its tmpdir. `tests/unit/ledger-isolation-discovery.bats` now treats an env-prefixed command (`KEY="" run "$CHEVAL" …`) as a spawn and accepts lowercase `$cheval` variables, so both shapes are caught going forward. Both ledgers were rotated per the hygiene runbook; the archive paths are in the Sprint 4 report.
- **Eval executor confinement (Sprint 3 audit HIGH-001)**: `evals/harness/execute-agent.sh` now runs the agent under test with `--restricted --tools Read,Grep,Glob,Write` — the exact SDD tool set, user/project settings ignored, file tools confined to the sandbox — and unsets the operator's non-CLI credentials (`GH_TOKEN`, `GITHUB_TOKEN`, `OPENAI_API_KEY`, `AWS_*`) with `TMPDIR` inside the sandbox; `evals/graders/implement-discipline.sh` runs agent-authored `test_command`s under `env -i`. Under the previous `--allowed-tools`, the operator's own `Bash(...)` allow rules leaked into the sprint-237 A/B arms (observed in the surviving transcripts) and 30 of 181 trials wrote helper files to `/tmp`; the recorded A/B stays valid as a same-environment comparison and is annotated as measured unconfined (bead bd-vq7v for a confined re-measure). Tests: EA-3 pins the confined argv, EA-10 the env hygiene, ID-10 the grader isolation.
- **Fixes surfaced by the first full unit run since the cycle started**: `tools/ceiling-probe.py` (cycle-109 fixture probe used by `loa-substrate-recalibrate.sh`) restored after Sprint 1 had overwritten it; the live probe is `tools/ceiling-probe-live.py`; `compare.sh` reads JSONL first rows correctly when writing baselines; model-config artifacts regenerated.
...
- Anthropic HTTP 404 whose message is `model: <id>` (model not served by the account) is chain-walkable instead of terminal; an endpoint/config 404 (`Not Found`) stays terminal.
- **Review round 1 (cycle-124 Sprint 1)**: the persona-less `--system` payload (Bridgebuilder's shape) now carries the prompt-cache breakpoint per PRD FR-4; the `max_tokens` stop-reason flag keys on the thinking class (Fable included) or a returned thinking trace; `--effort` reaches the `claude-headless` CLI hop (`CompletionRequest.effort` is the first candidate); the golden-path audit gate also runs the review trailer through `verdict-derive.sh` and denies on a non-integer `excluded`/`excluded_confirmed`; the cost-ledger resolver refuses existing non-regular targets and anchors relative config/default paths at the project root (env paths stay CWD-relative); `rollup.py` and `cost-report.sh` resolve their default through the writer's resolver over the merged config; `FLATLINE_SCORE_MAX_TOKENS` 4 000 → 16 000; `tools/regen-model-artifacts.sh` exits 3 on a missing toolchain (never "drift"); `tools/ceiling-probe.py` counts `message_stop` as OK with `max_tokens: 1024` and records `stop_reason`; Bridgebuilder `progressiveTruncate` clamps only ids the budget tables know; the temperature-drop warning fires once per model per process; the ledger-hygiene MODELINV rule matches `cheval-e2e-` without the `/tmp/` root; the cost-ledger readers fail closed like the writer (round-2 dissent); `live-floor-check.yml` is `workflow_dispatch`-only and targets the `live-floor` environment — it no longer hands `ANTHROPIC_API_KEY` to a pytest step on `pull_request` (audit dissent). Audit slices: test ledger isolation is unconditional; the daily-spend sidecar opens with `O_NOFOLLOW`; readers refuse rather than guess when the merged config cannot load; the non-streaming read timeout follows the output budget; a model-not-found 404 (`ModelNotFoundError`) no longer counts against the provider breaker; Bridgebuilder's adaptive retry derives from the clamped budget (`effectiveInputBudget`); the v3 schema types the pricing fields.
