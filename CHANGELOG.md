...
- **The cost-budget enforcer measures the day it certifies** (sprint-bug-245, bead `bd-ypbg`) — the `halt-uncertainty: unpriced_share` guard compared the ledger's *all-time* unpriced share against 5 %, so a repository with historical unpriced rows (this one: 37 of 54, written before the pricing ladder) could never be certified `allow` again. `cost-report.sh --json --window-day <YYYY-MM-DD>` now reports that UTC day's rows and unpriced share as a `window` object next to the unchanged all-time fields; the enforcer asks for the verdict's `utc_day`, compares the window share (an unpriced row inside the window still halts), falls back to the all-time share when the report has no window, and carries `unpriced_window` in the allow payload and `window_day` / `unpriced_share_all_time` in the halt diagnostic (payload schemas extended additively); the bats stub-script seam that exercises the argv also requires `LOA_BUDGET_TEST_MODE=1` (double gate). New explicit, opt-in `cost-report.sh --reprice [--dry-run]` re-prices the historical unpriced rows the ladder can now resolve exactly as fresh rows with the same tokens (`loa_cheval.metering.reprice`), marks each changed row (`repriced_at`, `repriced_from`, `cost_estimated: true`, `pricing_resolution`, `resolved_model`), keeps priced rows, unresolvable rows and corrupt lines byte-for-byte, refuses a symlinked ledger, holds the writer's `flock` on the ledger inode from read to last byte and rewrites the same inode in place (a concurrent `append_ledger` blocks, then lands after the new content — never on an orphaned inode), first copies the bytes read to `<ledger>.pre-reprice-<UTC>` through `O_CREAT|O_EXCL|O_NOFOLLOW` (a planted file or symlink is never followed) and writes `.run/cost-ledger-reprice-<UTC>.json` the same way, reserving the receipt name exclusively first (counts and sha256s, never row contents); only `(provider, model)` through the config-owned ladder decides a price (a row's own `resolved_model` is never an authority); a second run re-prices 0. The migration receipt now uses the same exclusive writer. Reports gain `repriced_rows`.
- **Run preflight P2 evaluates the settings layers Claude Code evaluates** (sprint-bug-246, bead `bd-n7v3`) — `check-permissions.sh` read only `.claude/settings.json` and matched rules by grepping the file's text, so run-mode allow rules approved into `.claude/settings.local.json` (the file Claude Code writes on approval) or set in `~/.claude/settings.json` were invisible, and a pattern that appeared only inside a `deny` array counted as allowed. It now reads `~/.claude/settings.json`, `.claude/settings.json` and `.claude/settings.local.json` as JSON arrays (a malformed file is skipped with a `WARN`, allowing and denying nothing), takes the union of the allow rules, lets a deny rule in any layer win (exact rule or the base wildcard `Bash(<cmd>:*)`; a narrower deny for a specific dangerous form does not deny the generic `Bash(<cmd>:*)` requirement), names the files it consulted (`settings_files`) and the denied rules with the file that denies them (`denied[]`, `total_denied`), and takes `--root <dir>`. Exit codes keep their meaning (0 effective · 1 missing or denied · 2 no settings file in any layer); `--quiet` is exit-code only (warnings silenced too). `run-preflight.sh` P2 names the layers and the deny rule in its detail and fix text.

## [2.0.0-rc.2] — 2026-09-24 — Friction floor (release candidate)

Second candidate of the 2.0 line, still a **pre-release**. The cycle-125 arc (sprints 241–244, PR #1269) removes the friction the fleet's own sessions showed most often, measured across 4.9 GB of local transcripts from 40 mounts (`grimoires/loa/reports/usage-mining-2026-09-23.md`): fence false positives, oversized planning artefacts, runs that start and cannot finish, opaque provider state, and unpriced model ids. No new configuration key; every fence relaxation is corpus-gated; the rc exit criteria are unchanged (`grimoires/loa/reports/release-notes-2.0.0-rc.1.md`, notes for this candidate in `release-notes-2.0.0-rc.2.md`).

### Changed

- **Provider health at a glance** (cycle-125 sprint 4, PRD FR-4) — `/loa` gains a `Providers` block (also under `--json .providers`): per provider the credential *presence* (never the value), the CLI hop on PATH, and every breaker bucket with state, age and probe timing (`OPEN 2h (probe overdue → HALF_OPEN on next call)`). New operator CLI `python3 -m loa_cheval.routing.breaker_cli --list [--json] | --reset PROVIDER[:AUTH_TYPE]` and `cheval --reset-breaker PROVIDER[:AUTH_TYPE]`; a reset writes an `operator_reset` journal marker before the state file. An OPEN bucket is proven to walk the within-company chain to the CLI hop without calling the provider (`test_breaker_open_walks_to_cli_hop.py`). `grimoires/loa/known-failures.md` is now seeded from `.claude/templates/known-failures.md.template` by `mount-submodule.sh` and `mount-loa.sh` (never overwritten); `check-loa.sh` warns when it is absent.
- **Cost accounting sees the ids the fleet actually calls** (cycle-125 sprint 4, PRD FR-5) — `find_pricing` resolves exact → dated (`gpt-5.5-2026-04-23` → `gpt-5.5`) → alias (`fable` → `claude-fable-5-1`) → CLI hop (`codex-headless` → its `extra.cli_model`), and a config-priced ledger row carries `pricing_resolution`; headless adapters record `transport` (`cli:claude`, `cli:codex`) and `resolved_model` on the result; a row priced from cheval-counted tokens is `cost_estimated: true`; absent pricing stays `pricing_source: unknown` at cost 0 — counted, never dressed as a price. `cost-report.sh` prints `Unpriced rows: N (S %)` plus `pricing_resolution` counts (JSON `unpriced_rows`, `unpriced_share`, `estimated_rows`, `legacy_rows`), reads the pre-2.0 ledger with `--include-legacy` (de-duplicated by `request_id`) and migrates it with `--migrate-legacy` through the resolver-validated writer, tagging rows `legacy: true` and writing a receipt `.run/cost-ledger-migration-<UTC>.json` (paths, counts, hashes; idempotent). The cost-budget enforcer refuses to certify `allow` while the unpriced share exceeds 5 % (`halt-uncertainty`, reason `unpriced_share`); an unavailable report is reported as unknown and does not block.
- **Run preflight, task checkpoints and resume surfacing** (cycle-125 sprint 3, PRD FR-3) — new `.claude/scripts/run-preflight.sh [--unattended] [--resume] [--json]` runs at the entry of `/run`, `/run sprint-plan`, `run-bridge` and `/run-resume` and fails loud with a checklist naming each predicate and its fix: P1 effective `permissions.defaultMode` (bypass passes; acceptEdits/default pass when the allow rules are present; `plan` and `auto` fail unattended), P2 allow rules, P3 voices (credential present or CLI hop on PATH per flatline stage; values never printed), P4 provider breakers, P5 NOTES size, P6 run state (stale `RUNNING`/`INTERRUPTED`/`HALTED` → the resume command), P7 beads, P8 branch. `.run/sprint-plan-state.json` gains `schema_version` 2 with a `checkpoint {sprint, task, phase, ts}` written atomically under a lock by `run-checkpoint.sh` after each task close and phase change; `read` discards a checkpoint whose bead is not closed (beads stay the recovery truth). A new SessionStart hook `loa-run-state-surface.sh` (behind `hook-guard.sh`, in both settings files) prints `Run: <state> (<age>) → /run-resume` for a resumable run and a session-limit-reset line; `/loa` shows the same `Run:` line and `workflow-state.sh` suggests `/run-resume` ahead of the phase command. Re-entry proof: `tests/integration/implement-reentry.bats` shows `br ready` skipping closed beads against the real `br`. No new config key.
- **Sectioned planning artefacts** (cycle-125 sprint 2, PRD FR-2) — `notes-guard.sh read --file <prd|sdd|sprint|NOTES>.md --section <spec>` returns one H2 block under the 68 KiB cap, where `<spec>` is `Sprint N`, a numbered SDD section (`4` / `4.`) or a case-insensitive heading substring; `--index` lists every H2 as `L<start>-L<end>  <bytes>B  <heading>`; a miss prints the headings and the index (never empty); for any file other than NOTES.md the flag-less default is the index. The shared `context_discipline` skill include now routes big artefacts through it (net −0 B against the skill budgets, with compensating trims in `implementing-tasks`, `reviewing-code`, `auditing-security`), `/implement` reads its sprint block by section, `/loa` prints an `Artefacts:` line with the four sizes and a ≥ 100 KiB warning, and `update-loa.sh` rotates a NOTES.md that is at or over the 200 KiB block line after a refresh (archive-first, logged; below the line nothing happens). Migration guide: rc.2 addendum.
- **Destructive-command fence precision** (cycle-125 sprint 1, PRD FR-1) — `block-destructive-bash.sh` stops four false-positive classes measured across the fleet's sessions without losing a genuine catch: `rm -rf` on build/cache directories by name (`dist`, `node_modules`, `.venv`, …), on bare names after a statement-initial `cd /tmp/…`, on paths strictly below `/tmp`, `/var/tmp`, `/private/tmp`, on `$TMPDIR/<name>` with a real temp `TMPDIR`, and on a variable bound exactly once to `$(mktemp -d)`; `git branch -D` on a branch that is already an ancestor of main/master (offline `git merge-base`); `git checkout -- <generated path>` (build outputs, lockfiles, `linguist-generated`); and the DROP / TRUNCATE / DELETE rules only when a SQL runner or an inline interpreter program is in the command. Catastrophic and hidden paths, `..` escapes, remote payloads (`ssh host 'rm -rf /'`) and the hook's own `$PWD` are unchanged. New sanctioned helper `.claude/scripts/git-branch-prune.sh [--dry-run] [--base <ref>] [--json]` deletes merged, squash-merged (`gh` probe with `timeout 5`, `LOA_FENCE_NO_NETWORK=1` opt-out) and gone-upstream branches with a restore hint. The committed corpus `tests/fixtures/fence-corpus/corpus.jsonl` (49 benign, 60 dangerous, 5 residual at the end of the cycle; every bypass found in review or audit was added as a row) is a gated regression floor: benign ≥ 80 % (14/46 at the start of the cycle, 49/49 now), dangerous 100 %, runtime ≤ 1.5× the per-row baseline.

### Fixed

- **Bridgebuilder pass on PR #1269** — `run-checkpoint.sh read` discards a task checkpoint when `br` is unavailable instead of trusting it (a checkpoint is trusted only when beads confirms the task closed); `cost-report.sh` reports a pre-metadata ledger row that already carries a cost as `unclassified_rows`, not as unpriced, so historical priced spend never inflates `unpriced_share`; the budget writer accepts stubbed completion results (`resolved_model` / `transport` reach the ledger only as non-empty strings).

## [2.0.0-rc.1] — 2026-09-23 — Model-generation floor (release candidate)

First cut of the 2.0 line, published as a **pre-release** so downstream mounts can soak it: fixes land as `-rc.N`, promotion to `2.0.0` follows the exit criteria in `grimoires/loa/reports/release-notes-2.0.0-rc.1.md`. The cycle-124 arc (sprints 235–238, PR #1266) brings the framework's own use of Claude to the Opus 5 / Sonnet 5 / Fable 5.1 generation and rolls up the unreleased upstream work since [1.202.1]. Read first: `docs/migration/v2.0-model-generation-floor.md` (recipes and kill switches), `docs/architecture/ADR-004-model-generation-floor.md` (decisions), `grimoires/loa/reports/breaking-surface-audit-2.0.0-rc.1.md` (every change at its line).

### Breaking

- **Anthropic per-hop `max_tokens` default** 4096 → 16 000 non-streaming / 64 000 streaming, clamped to the catalog `max_output_tokens`; `--max-tokens 0` is refused (`INVALID_INPUT`). Kill switch `LOA_CHEVAL_LEGACY_WIRE=1`; explicit `--max-tokens N` is honoured (clamped, never raised).
- **Adaptive thinking on the wire** — `thinking: {type: adaptive}` for catalog entries flagged `params.thinking_adaptive` (Opus 5 / 4.8 / 4.7 / 4.6, Sonnet 5 / 4.6), omitted for Fable. `LOA_CHEVAL_LEGACY_WIRE=1` restores the pre-cycle request body byte-for-byte (thinking, `cache_control` and `output_config.format` together).
- **Model floor** — `opus` → `claude-opus-5`, `fable` → `claude-fable-5-1`; `hounfour.models.opus` example retargeted. Old ids stay in the catalog and resolve by explicit pin; `LOA_FORCE_LEGACY_ALIASES` is a cycle-095 snapshot and does not restore the pre-2.0 targets.
- **Forced `tool_choice`** (`required`, unknown values) raises `InvalidInputError` on the Anthropic adapter instead of silently mapping to `any` / `auto`; use `auto`, `none` or a named tool.
- **Dissent strictness** — schema-enforced dissent payloads are parsed strictly (`malformed_response`, `parse_path: schema_enforced`) and a review or audit with any rejected finding is DEGRADED unconditionally; the `flatline_protocol.dissent.repair_loop` key is removed (a stale key is ignored, never an error).
- **`effort:` config block removed** (it was never read); effort comes from SKILL.md frontmatter and `--effort`; `xhigh` maps to `high` on the 4.6 generation and is omitted for Sonnet 4.5 / Haiku 4.5.
- **Cost ledger** — default `hounfour.metering.ledger_path` moves to `.run/cost-ledger.jsonl`; `LOA_COST_LEDGER_PATH` > config > default; the writer refuses symlinks, non-regular files and missing parents (`INVALID_CONFIG`); `rollup.py` / `cost-report.sh` fail closed instead of guessing. Old history: `--ledger grimoires/loa/a2a/cost-ledger.jsonl`.
- **Three protocols archived** to `grimoires/loa/archive/protocols/` (`risk-analysis.md`, `sprint-completion.md`, `upgrade-process.md`; the verdict-trailer rule lives in `feedback-loops.md`); ten `.constraint-*` twins deleted and the shape rejected by `no-backup-files.yml`.
- **Bounded NOTES.md** — 100 KiB warn / 200 KiB block on growth across `notes-guard.sh check`, the `notes-size-guard.sh` PreToolUse hook, `FR-NOTES` (`>>` appends) and `update-notes-learnings.sh`; Level-1 recovery reads `notes-guard.sh read` (≤ 68 KiB). Shrinking edits and `rotate` are never blocked; there is no bypass variable.
- **Aleph is opt-in** — `aleph.enabled` (default `false`; `LOA_ALEPH_ENABLED=1` for tests/CI) in front of the mount/update refresh, `check-loa.sh`, the three Aleph suites and both Aleph workflows. Aleph users set `aleph.enabled: true` before upgrading; the opted-in behaviour is unchanged (bug 20260922-13a3d1, sprint-bug-239).
- **Post-merge pipeline prepares, never publishes** (upstream `2af691ac`): the merge produces an inspectable `release-candidate-<sha>` artifact; publication is `post-merge-orchestrator.sh --publish FILE --approve-sha256 DIGEST` from a verified checkout (runbook `grimoires/loa/runbooks/post-merge-candidates.md`).
- **Flatline content-qualified quorum** (#1227, upstream): consumer gates require canonical `verdict_quality.status: APPROVED`; schema-invalid exit-0 prose no longer counts as a voice (`beads-flatline-loop.sh` exit 6, orchestrator exit 3).
- **Golden-path verdict gate** — a feedback file carrying a `LOA-VERDICT` marker is parsed by `verdict-derive.sh` and must be consistent (one-way rule; lowercase or malformed markers are violations; audit gates re-check the review trailer and `excluded` / `excluded_confirmed`). Legacy files with no trailer keep the prose heuristic.
- **Fail-loud tightenings from upstream** — safety hooks (`block-destructive-bash.sh`, `spiral-dispatch-guard.sh`, `zone-write-guard.sh`, `7a900437`) block more Bash shapes; `ledger-lib.sh` refuses empty or unparseable content writes and propagates the failure (`63d10ac1`, `e2a274eb`, KF-024).

### Added

- **Catalog at the current generation** — `claude-opus-5`, `claude-fable-5-1`, Sonnet 5 at $2/$10, the 4.6+ family at 1M context / 128K output, typed `params.thinking_adaptive` / `temperature_supported`, `structured_json`, `cache_read_per_mtok`, `effective_input_ceiling` + `ceiling_calibration`; `tools/regen-model-artifacts.sh` (`--check` for the drift gates). Values are `reference` until `live-floor-check.yml` records a pass.
- **cheval** — `--effort low|medium|high|xhigh|max`, `--json-schema FILE` (object root, ≤ 64 KB, canonical sha256; enforced as `output_config.format` on `structured_json` entries, forwarded to `claude-headless`, OpenAI `text.format` pass-through), `--dry-run` reports budget and effort; CLI JSON `schema_enforced` + `stop_reason`; `model-adapter.sh --json-schema` / `resolve_effort()`.
- **Prompt caching** — persona.md as the single `cache_control` breakpoint on every transport; `Usage.cache_*` → MODELINV `tokens_cache_read` / `tokens_cache_creation`, CLI JSON, ledger rows, `pricing_snapshot`; cache read/write priced at catalog rates. MODELINV payload also gains optional `schema_enforced` / `output_schema_sha256` (refresh pinned schema copies).
- **Wire schemas** `.claude/schemas/wire/{dissent-review,dissent-audit,flatline-reviewer,flatline-skeptic,flatline-scorer}.wire.json`, consumed by `adversarial-review.sh` and `flatline-orchestrator.sh`; synthetic fixture corpus under `tests/fixtures/structured-outputs/`.
- **Ledger isolation** — `LOA_COST_LEDGER_PATH`, `O_NOFOLLOW` writer, pytest/bats/TS harness isolation, `tools/check-ledger-hygiene.sh` tripwire in CI and `pre-push-audit`, rotation runbook; the unit suite no longer writes the production ledgers (KF-033).
- **Coverage-first review and audit** — every finding with `file:line`, failure scenario, severity and confidence; minimum-count quotas removed; `verdict-derive.sh` `excluded` / `excluded_confirmed`; effort defaults from SKILL.md frontmatter; Flatline reviewer/skeptic at `xhigh`, scorer at `medium`.
- **Prompt byte budgets** — `tools/check-prompt-budget.sh` (skills ≤ 16,384 B incl. unconditionally-read resources, `CLAUDE.loa.md` ≤ 10,240 B, protocols ≤ 200,000 B), `.github/workflows/check-prompt-budget.yml` on every PR (fails loud without a report), `tools/prompt-keeplist.txt`, the 49-unit prompt audit, `prompt-auditor` / `prompt-auditor-io` agents.
- **Bounded session memory** — `notes-guard.sh check | read | rotate`, the hook, the fence and the writer gate; decision memo `grimoires/loa/reports/2026-09-17-notes-vs-memory-tool.md`.
- **Eval A/B harness** — `evals/harness/execute-agent.sh` (headless `claude -p` under `--restricted --tools Read,Grep,Glob,Write`, tree sha and dirty flag per trial), a 10-PR review corpus, implement-discipline fixtures, deterministic graders, `compare.sh --ab` gates; Sprint 3 A/B recorded under `record/cycle-124-a2a`.
- **Live floor** — `.github/workflows/live-floor-check.yml` (`workflow_dispatch`, environment `live-floor`, `main` / `release/*` only, fork refusal, missing key fails loud, no raw output while the key is present), `tests/replay/test_cycle124_live_floor.py`, `tools/ceiling-probe-live.py`.
- **Pre-release candidates through the pipeline** (bug 20260923-27d899, sprint-bug-240) — `semver-bump.sh` honours the topmost untagged CHANGELOG heading for two transitions (enter `2.0.0` → `2.0.0-rc.1`, promote `2.0.0-rc.N` → `2.0.0`; `rc.N → rc.N+1` unchanged; any other heading ignored, a mismatched rc heading warns) and reports `prerelease_transition`; the candidate carries `prerelease`, `publish_candidate` ties it to the tag, `phase_release` publishes `prerelease: true` and verifies it on read-back; `sync-readme-version.sh` accepts `X.Y.Z-PRE.N` with the shields `--` badge escape.
- Upstream since 1.202.1: installer download verification and relocation data (`c9c37398`), audit envelope bound to authenticated bytes (`7a3137b3`), shared headless adapter flow with CLI cost accounting (`866e5008`), validation/dispatch bound to the owning scope (`4714113a`), portable ledger locks and refreshed evidence.

### Changed

- `CLAUDE.loa.md` is a 10,222-byte kernel (Karpathy principles' full text in `karpathy-principles.md`); 13 skills cut to the floor with conditional procedures behind guarded `resources/*.md` pointers; protocols 267 KB → 199 KB.
- Bounded dispatchers pass explicit budgets (Flatline 16K/4K, adversarial dissent 16K, Bridgebuilder 16K); the dissent input budget follows the dissenter's company (Anthropic 160K); Bridgebuilder truncation consumes the generated per-model budgets.
- Anthropic HTTP 404 `model: <id>` is chain-walkable (an endpoint 404 stays terminal); a model-not-found 404 no longer counts against the provider breaker; the non-streaming read timeout follows the output budget.
- The cycle's per-sprint a2a record lives on the never-merged branch `record/cycle-124-a2a`; the template repository keeps `grimoires/loa/a2a/sprint-*` and `a2a/trajectory/` out of `main` by policy.

### Fixed

- Golden-path review/audit gates derive the verdict from the trailer and fail closed on inconsistency; trailer detection is case-insensitive and byte-exact on the canonical form; `ledger-lib.sh` walks skip bug-fix cycles' label-string entries; `validate-ac-verification.sh --sprint-id` scopes to one sprint.
- cheval: `--effort` reaches the `claude-headless` hop; the `max_tokens` stop-reason flag keys on the thinking class; `--json-schema ""` is `INVALID_INPUT`; the CLI schema-rejection retry keys on the CLI's stderr; whole-number float cache rates accepted; the temperature-drop warning fires once per model per process; cost-ledger readers fail closed like the writer.
- `adversarial-review.sh`: at most 5 repair round-trips per run (`repair_budget_exhausted`), exactly one JSON object on the enforced branch, WARN on a missing wire schema, prompts fed to `jq` via `--rawfile`; `flatline-orchestrator.sh` rejects an enforced voice that stopped at `max_tokens` / `refusal` (`enforced_truncated`); Flatline `--help` budgets corrected.
- `tools/ceiling-probe.py` restored (Sprint 1 had overwritten the cycle-109 fixture probe; the live probe is `tools/ceiling-probe-live.py`); `compare.sh` baseline first-row read; model-config artifacts regenerated; stale test pins repaired (`test_golden_path.bats`, `model-adapter-overlay-source.bats`, `cheval-input-gate.bats`, KF-006 v3 migrator smoke, `bug-881` context window).
- CI: the backup-sibling check ignores deletions (`--diff-filter=d`); ledger isolation discovery recognises indirect spawners and env-prefixed commands; two suites that reached cheval for real now run through mock fixtures.
- Upstream: post-merge preparation preserves sourced script modes (`90929097`); `ledger-lib.sh` refuses the 1-byte blank write (KF-024); scheduled workflows and test gates repaired.

### Security

- Eval executor confinement (Sprint 3 audit HIGH-001): `--restricted --tools Read,Grep,Glob,Write`, operator user/project/local settings ignored, `GH_TOKEN` / `GITHUB_TOKEN` / `OPENAI_API_KEY` / `AWS_*` unset, `TMPDIR` inside the sandbox; graders run agent-authored test commands under `env -i`.
- `live-floor-check.yml` never hands `ANTHROPIC_API_KEY` to PR-controlled code: dispatch-only, trusted refs and environment branch policy, artifact validation by key shape before upload, no raw test output in the log while the key is present.
- Cost-ledger and daily-spend writers open with `O_NOFOLLOW` and refuse non-regular targets; readers refuse rather than guess when the merged config cannot load.
- Upstream: audit envelope verification and consumption bound to authenticated bytes; installer downloads verified; safety hooks preserve command boundaries and resolve repository paths.
