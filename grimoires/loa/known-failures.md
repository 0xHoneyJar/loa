...
| 2026-09-17 | cycle-124 T1.3/T1.4 re-baseline: Anthropic entries drop the v2 input fields for the v3 effective_input_ceiling (180K, kf_derived, streaming-probed); the 36K non-streaming wall is now cheval's _LEGACY_TRANSPORT_INPUT_WALL applied under LOA_CHEVAL_DISABLE_STREAMING; per-hop output default min(64K streaming\|16K legacy, catalog max_output_tokens) replaces the 4096 literal; bounded dispatchers pass explicit budgets; AC-3.5 matrix (test_input_size_consumers.py) pins which gate fires first for 120K..900K inputs | RE-BASELINED — the layer-3 gate is a catalog field + constant instead of three per-entry numbers; observed: the two gates disagree on exit code (pre-flight 7 vs legacy wall 12) for the same refusal class, recorded for a follow-up | commits f5ea53f4 + e14198a1 (feature/cycle-124-model-generation-floor) |
...
| 2026-09-17 | cycle-124 Sprint 1 security dissent (adversarial-review.sh --type audit, gpt-5.5-pro chain under cli-only) returned one finding without a string id; rejected by the schema gate, repair loop did not recover it, envelope recorded as reviewed/0 findings | OBSERVED — FR-7 (Sprint 2) enforces output_schema on the dissenter wire and measures schema_enforced per voice; this row is the pre-FR-7 baseline sample | grimoires/loa/a2a/sprint-235/adversarial-audit.json (local a2a; branch feature/cycle-124-model-generation-floor) |
| 2026-09-17 | cycle-124 Sprint 1 audit dissent: the envelope said reviewed / 0 findings while adversarial-rejected-audit.jsonl held one HIGH payload rejected for missing-or-non-string-id (live-floor-check.yml handed ANTHROPIC_API_KEY to a pytest step on pull_request). Lead read the sidecar, accepted the finding, fixed the workflow (workflow_dispatch-only + live-floor environment). | RECOVERED-FROM-SIDECAR (finding acted on; omission class unchanged until FR-7 structured outputs land in Sprint 2) | commit 44e1feab on feature/cycle-124-model-generation-floor; grimoires/loa/a2a/sprint-235/adversarial-rejected-audit.jsonl (2026-09-17T22:06:08Z) |
| 2026-09-18 | cycle-124 Sprint 2 (FR-7) structured outputs landed: wire schemas + cheval --json-schema enforced on Anthropic structured_json entries and forwarded to claude-headless; adversarial-review.sh parses enforced payloads strictly (parse_path=schema_enforced, no repair) and keeps the tolerant path + repair loop (now flag-less) for unenforced voices; DEGRADED on any rejected finding unconditional. Live measurement on this host 2026-09-18 (one Flatline spec run + one review dissent): claude-headless 3/3 calls schema_enforced=true, codex-headless 2/2 false (schema not forwarded to codex/gemini headless — follow-up bead). First live run also caught the CLI rejecting the 2020-12 $schema meta-URI (fixed: 0c727a98). | STRUCTURAL-MITIGATION-SHIPPED (enforced hops cannot produce the silent-rejection shape; unenforced hops keep the sidecar + repair loop; the omission class persists only on hops that cannot enforce — codex/gemini headless, non-structured_json entries) | commits f6646c9e 02a79256 66792935 4e60888b 7306f43c cdf6768f 0c727a98 150868a5 on feature/cycle-124-model-generation-floor; MODELINV rows 2026-09-18T23:10Z–23:16Z (archived by the Sprint 2 rotation); tests/unit/adversarial-review-schema-enforced.bats FR7-1..10 |
| 2026-09-21 | Correction of the 2026-09-18 measurement row (late Sprint 2 review, evidence slice): re-running the one-liner on .run/archive/model-invoke-20260918T231348Z.jsonl (26 JSON rows + the rotation seal line, so grep '^{' before jq) gives claude-headless 3/3 schema_enforced=true (holds) and codex-headless 10/10 false (4 adversarial-review, 2 flatline-review, 2 flatline-skeptic, 1 flatline-score, 1 adversarial-audit) — the row's 'codex-headless 2/2' undercounted the unenforced voices | OBSERVED — ratio stands: every hop that can enforce did; every codex-headless hop ran unenforced; the repair-loop retirement bead keys on this corrected table | a1337ed4 (late Sprint 2 fixes, slice B); archive model-invoke-20260918T231348Z.jsonl |
| 2026-09-22 | Sprint-237 audit dissent (--type audit, gpt-5.5-pro via codex-headless, 4 diff chunks): 2+2 findings emitted, all dropped as missing-or-empty-failure_mode; repair round-trip did not add the field. Chunk 1's two payloads were lost because the sidecar is truncated per run (adversarial-review.sh:1171 ': > $rejected_sidecar'); the log line prints the post-repair reason (missing-or-empty-description), not the original. | Findings recovered from the chunk-4 sidecar and evaluated by the lead (both pre-existing text); chunk 1 re-run came back clean. Bead filed for per-run sidecar naming + original reason in the log. | grimoires/loa/a2a/sprint-237/adversarial-rejected-audit.4-exec-prompts.jsonl; adversarial-audit.{1-evals,4-exec-prompts}.json rejected_count=2; commit d562d510 tree |
...
**Recurrence count**: ≥6 (documented Attempts rows; true count higher — every PR since `dd54fe9c` that touches `model-config.yaml` or related paths; workflow only triggers on PRs, not main pushes — so main is "passing" by virtue of not running; normalized cycle-121)
...
| 2026-09-17 | cycle-124 T1.3: production model-config.yaml now carries v3 fields (effective_input_ceiling, ceiling_calibration, typed params) that the closed v2 schema cannot hold — fourth occurrence of the class; production smoke (cycle099-sprint-1e-tests.yml) and migrate-model-config.bats M19.3 flipped to --to-v3 + v3 schema; v2 remains for synthetic fixtures | RESOLVED-STRUCTURAL for the production path — the v2 smoke against a v3-shaped catalog is retired rather than re-patched; v2 schema no longer gates production | commit f5ea53f4 (feature/cycle-124-model-generation-floor) |
