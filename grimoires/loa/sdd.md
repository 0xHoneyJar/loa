...
`_persona_messages(agent, system_override)` NEW in `cheval.py` (sibling of `_load_persona`, which stays for the string-returning callers): returns `[{role:system, content:persona, cache_control:{type:ephemeral}}, {role:system, content: CONTEXT_SEPARATOR + CONTEXT_WRAPPER_START + context + CONTEXT_WRAPPER_END + PERSONA_AUTHORITY}]`, or one **unmarked** message when there is no persona; `"".join(contents) == _load_persona(...)` is a pinned invariant. Non-Anthropic adapters' system transformers concatenate N system messages exactly as they concatenate today and ignore `cache_control` (golden body tests, AC-4.4).
