...
Catalog untrusted entry points (sources) and dangerous sinks without investigating yet — the source, sink and trust-level lists: `resources/REFERENCE.md` §Sources and Sinks. Track them in a working file, e.g. `grimoires/loa/a2a/audits/YYYY-MM-DD/SECURITY_ANALYSIS_TODO.md`, with file:line, trust level and a status (`PENDING` → `CONFIRMED` / `SAFE` / `PARTIAL` / `N/A`) per item. On a large repo, prioritize by sink severity and route reachability; cap entries and log overflow rather than stalling.
