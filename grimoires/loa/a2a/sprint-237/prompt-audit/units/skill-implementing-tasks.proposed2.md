...
Tests assert specifics ("returns 200, response includes `user.id`"), evidence is file:line (`src/auth/middleware.ts:42-67`, not "updated auth"), reproduction is the exact command (`npm test -- --coverage --watch=false`). Cite sprint task IDs and SDD sections for architectural decisions; quote feedback items verbatim when addressing them.

Before handoff, run the same fast checks CI runs — linter, tests and, when the project configures them, the formatter in check mode (`prettier --check`, `ruff format --check`, `gofmt -l`, `cargo fmt --check`, …) and the type checker (`tsc --noEmit`, `mypy`, `pyright`, `go vet`, …). A formatter that would rewrite files is a red CI run waiting to happen; treat a format or type failure like a failing test.
</kernel_framework>

<grounding_requirements>
Before implementing, in order:
1. `grimoires/loa/a2a/sprint-N/auditor-sprint-feedback.md`, if present: `CHANGES_REQUIRED` means fix every CRITICAL/HIGH issue (MEDIUM/LOW if feasible) and record each issue with its fix and verification in a "Security Audit Feedback Addressed" report section; `APPROVED - LET'S FUCKING GO`, or no file, means proceed.
...
3. `grimoires/loa/a2a/integration-context.md`, if present: context-preservation links, documentation locations, commit-message formats, available MCP tools.
...
Section present → proceed to Phase -2. Absent with config `false` → proceed and log `{"phase":"karpathy_check","principle":"goal_driven","verdict":"skipped_by_config",...}` to `grimoires/loa/a2a/trajectory/karpathy-{date}.jsonl`. Absent with config `true` (default) → **AskUserQuestion** before any tool call, offering: provide criteria now (append a "Success Criteria" section to sprint.md, then proceed), skip with a one-line rationale (log and proceed), or abort (no tool calls; re-invoke after updating sprint.md).
...
Full specification: `.claude/protocols/beads-preflight.md` and `.claude/protocols/beads-integration.md`; command reference (`get-ready-work.sh`, label table): `resources/REFERENCE.md` §Beads Workflow.

## Phase -1: Context Assessment and Parallel Task Splitting

Assess context size first: `wc -l grimoires/loa/prd.md grimoires/loa/sdd.md grimoires/loa/sprint.md grimoires/loa/a2a/*.md 2>/dev/null`. Under 3,000 lines is SMALL — sequential, proceed to Phase 0. 3,000–8,000 is MEDIUM — parallel when 3+ independent tasks. Over 8,000 is LARGE — split. MEDIUM/LARGE: see `resources/REFERENCE.md` §Parallel Implementation Guidelines for the dispatch and consolidation rules.
...
Monitor for feedback files. When one arrives, read it fully, address every item (ask specific clarifying questions when an item's meaning is unclear — in an unattended run, record the interpretation in NOTES.md and proceed), and regenerate the report with a "Feedback Addressed" section. Document interpretations and technical tradeoffs in the report for the reviewer.
</workflow>

<checklists>
Checklists (Pre-Implementation, Code Quality, Testing, Documentation, Versioning), the SemVer bump table, and the Task Plan Template live in `resources/REFERENCE.md`. A complex task (3+ files or modules, an architectural decision, an unclear implementation path, multiple acceptance criteria, or security-sensitive work) gets a plan before code at `grimoires/loa/a2a/sprint-N/task-{N}-plan.md`, checked against the SDD and, if high-risk, approved by a human; it becomes part of the review artifact. Simple tasks (docs, config, small fixes) skip planning.
