# Prompt-audit brief — unit `persona-flatline-attacker`
...
- **File**: `/home/merlin/Documents/thj/code/loa/.claude/skills/flatline-attacker/persona.md` (6226 bytes today)
- **Kind**: persona
...
- **Byte target**: none — personas are already small. Audit for dated patterns only (pressure language, choreography, restated defaults); the JSON schema block, field list, minimal valid example and the `## Coverage` section are contract and stay byte for byte.
...
- `/home/merlin/Documents/thj/code/loa/grimoires/loa/a2a/sprint-237/prompt-audit/units/persona-flatline-attacker.report.md` — the report (markdown, at most ~4,000 characters)
- `/home/merlin/Documents/thj/code/loa/grimoires/loa/a2a/sprint-237/prompt-audit/units/persona-flatline-attacker.proposed.md` — the complete proposed file, byte-exact (no fence, nothing else)
- `/home/merlin/Documents/thj/code/loa/grimoires/loa/a2a/sprint-237/prompt-audit/units/persona-flatline-attacker.resource-<NAME>.md` — one per moved resource (skills only; NAME = the file name under `resources/`)

Write each with a quoted heredoc so nothing expands: `cat > "<path>" <<'LOA_EOF'` … `LOA_EOF`. Verify with `wc -c` that the proposed file has the byte count you intend. Bash is for these saves only — never touch the audited file, `.claude/`, git or the network. Your final message is one line: `done persona-flatline-attacker <proposed-bytes>`.
