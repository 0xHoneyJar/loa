...
- [ ] **T1B.4** ⚠ HIGH (Reliability) — Apply per-model `max_output_tokens` lookup to `.claude/scripts/model-adapter.sh` (T1.9 Sprint 1A only covered `model-adapter.sh.legacy`; the non-legacy path is what `adversarial-review.sh` actually invokes). Mirror the `_lookup_max_output_tokens` helper from `model-adapter.sh.legacy` (or extract to shared lib `.claude/scripts/lib/model-adapter-shared.sh`); wire at the same three call sites (call_openai_api, call_anthropic_api, call_google_api equivalents); add bats coverage pinning payload `max_output_tokens` field via curl-mock harness ([#808](https://github.com/0xHoneyJar/loa/issues/808) dependency); regression-test by re-running `adversarial-review.sh` against this branch and confirming non-empty content. Source: cycle-102 Sprint 1A round-2 review — Phase 2.5 adversarial review hit empty-content × 3 retries on gpt-5.5-pro on 67k-token diff (the exact bug class T1.9 was meant to fix). The /reviewing-code Phase 2.5 gate currently fails open via api_failure across all future cycles until this lands. **Recommended priority: ship FIRST in 1B (1-2h scope) so subsequent 1B work has a working adversarial gate.**
