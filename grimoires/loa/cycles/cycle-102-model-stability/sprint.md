...
- [ ] **T1B.4** ⚠ HIGH (Reliability) — **REFRAMED 2026-05-09 — original framing was wrong.** Sprint 1B kickoff verified: `model-adapter.sh` is a routing shim that `exec`s `model-adapter.sh.legacy` when `hounfour.flatline_routing: false` (the default), so T1.9's fix DOES apply through this path. `_lookup_max_output_tokens openai gpt-5.5-pro 8000` returns 32000 correctly. **Actual root cause**: gpt-5.5-pro returns empty content for ≥27K-token inputs at `reasoning.effort: medium` even with `max_output_tokens=32000` — a deeper scale-dependent layer of vision-019 that the T1.9 threshold (verified at 10K) doesn't cover. **Operator decision needed**: (a) bump default to 64K (untested at this scale); (b) swap `flatline_protocol.code_review.model` to `claude-opus-4-7` (lowest-risk; supersedes T1B.4 if landed); (c) drop adversarial-review.sh truncation budget below the empty-content threshold; (d) lower `reasoning.effort` to `low` for dissent role. See `grimoires/loa/NOTES.md` Decision Log 2026-05-09 (cycle-102 Sprint 1B kickoff — T1B.4 ROOT-CAUSE REFRAME) for full investigation. Sprint 1B autonomous run HALTED on this discovery; resume via `/run-resume` after operator decision.
