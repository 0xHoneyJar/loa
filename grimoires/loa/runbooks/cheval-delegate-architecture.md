...
| `LOA_CHEVAL_LEGACY_WIRE` | unset | cycle-124 FR-1/FR-4: when `=1`, the Anthropic adapter emits the pre-cycle-124 request body byte-for-byte — no `thinking` block, no `cache_control` system blocks, no `output_config.format` (Sprint 2) — and cheval's per-hop `max_tokens` default drops back to 4096. A one-shot operator backstop while the live floor check is unprobed; the rollback policy is still `git revert`. Two limits (Sprint 1 review/audit): temperature omission is catalog-driven (`temperature_supported: false`), not restored by the switch; and on Opus 5 / Sonnet 5 thinking is default-on server-side (always-on on Fable), so omitting the block does not turn it off — with the 4096 default those ids can spend the budget on reasoning (the `stop_reason=max_tokens` warning still fires). Pass `--max-tokens` explicitly when using the switch on 5.x ids. |
| `LOA_COST_LEDGER_PATH` | unset (→ `metering.ledger_path` → `.run/cost-ledger.jsonl`) | cycle-124 FR-6: redirects the cost ledger writer AND every reader (`rollup.py`, `cost-report.sh`) to the given path; realpath-canonicalised, symlink targets refused, parent must exist. Test harnesses set it (with `LOA_MODELINV_LOG_PATH`) so suites never append to the repo's ledgers — `tools/check-ledger-hygiene.sh` is the tripwire. |
| `LOA_MODELINV_LOG_PATH` | unset (→ `.run/model-invoke.jsonl`) | Redirects the MODELINV audit-chain writer (and the consumers that take a path) — the isolation twin of `LOA_COST_LEDGER_PATH`. |
