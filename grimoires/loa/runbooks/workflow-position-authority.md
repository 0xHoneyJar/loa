...
- `golden-path.sh` is **already** file-presence-authoritative (`_gp_sprint_is_complete` / `_gp_sprint_is_audited` read markers, never `.run` state) — this runbook generalizes that to the resume paths.
